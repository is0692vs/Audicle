...
BEGIN
    ...
    IF array_length(add_playlist_ids, 1) > 0 THEN
        FOREACH playlist_id IN ARRAY add_playlist_ids LOOP
            INSERT INTO playlist_items (article_id, playlist_id)
            VALUES (article_id_param, playlist_id)
            ON CONFLICT (article_id, playlist_id) DO NOTHING;
        END LOOP;

        -- Count actually added items
        SELECT COUNT(*) INTO added_count_var
        FROM playlist_items
        WHERE article_id = article_id_param
        AND playlist_id = ANY(add_playlist_ids);
    END IF;
