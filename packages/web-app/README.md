...
- **Web Share Target API**: スマホの「共有」メニューから記事を直接追加（PWA）
...
1. **スマホの Chrome で記事を開く**
2. **「共有」メニュー** から **「Audicle」** を選択
3. **自動的に記事がプレイリストに追加** され、ウィンドウが閉じる
4. **PC で同じブラウザを開く** と、追加された記事がすぐに利用可能

> **注**: この機能は PWA (Progressive Web App) として動作します。スマホのブラウザで Audicle を開いたことがあれば、共有メニューに表示されます。
...
│   └── share-target/       # Web Share Target API エンドポイント
│       ├── page.tsx        # 共有エントリーポイント
│       ├── ShareTargetClient.tsx  # 記事追加ロジック
│       └── AutoCloseComponent.tsx # 自動クローズUI
├── components/
...
│   └── storage.ts          # 記事ストレージ
├── public/
│   └── manifest.json       # PWA マニフェスト (Web Share Target設定含む)
└── types/
    └── api.ts              # 型定義
```
